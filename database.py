...
class NewsDatabase:
        ...
        with self.get_connection() as conn:
            ...
            for i, article in enumerate(articles):
                try:
                    ...
                    sentiment, sent_confidence, analysis_text = self._enhanced_sentiment_analysis(
                        validated_article['title'], 
                        validated_article['description']
                    )
                    
                    # Get word count
                    word_count = len((validated_article['title'] + " " + 
                                     (validated_article['description'] or "")).split())
                    
                    # Insert article
                    cursor.execute('''
                        INSERT INTO articles 
                        (title, description, url, url_hash, published_at, source, 
            ...
            with self.get_connection() as conn:
                ...
                cursor.execute('''
                    ...
                    (content, content_hash, content_preview, model_used, article_count, 
                     processing_time, quality_score, raw_response_json, sentiment_summary, category_breakdown) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    content.strip(), 
                    content_hash, 
    ...
    def get_recent_articles(self, limit: int = 20, category: str = None, 
                          min_sentiment: float = None, max_sentiment: float = None) -> List[Dict]:
    ...
    def search_nodes(self, query: str, limit: int = 50) -> List[Dict]:
        """Search for articles based on query string"""
