...
import { 
  ...
  TrendingUp, Shield, Cpu, X, Edit2, Trash2, Copy, PanelLeftOpen, PanelLeftClose, Sparkles
} from 'lucide-react';
...
export function ChatWorkspace({ 
  ...
  const [useSearch, setUseSearch] = useState(true); // web-search backed model toggle
  const [streamingCompleteMessageIds, setStreamingCompleteMessageIds] = useState<Set<number>>(new Set());
  
  // Layout state: can toggle between inline and split
  const [layout, setLayout] = useState<'inline' | 'split'>(initialLayout);
          ...
          {activeConversation || isInline ? (
            <>
              ...
              <div className="border-t border-gray-200 dark:border-gray-700">
                {/* Smart Inline Header - Shows when messages exist in inline mode */}
                {isInline && hasMessages && (
                  <div className="flex items-center justify-between px-4 py-2 bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-750 border-b border-gray-200 dark:border-gray-700">
                    <div className="flex items-center gap-2">
                      <Button
                        variant="default"
                        size="sm"
                        onClick={() => handleNewConversation()}
                        className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white shadow-md transition-all duration-200 hover:shadow-lg"
                      >
