#!/usr/bin/env python3
"""
Geo-Political News → Investment Idea → Notifications Bot
...
class NewsBot:
    ...
    def fetch_news(self) -> List[Dict]:
        ...
        headers = {
            ...
            'User-Agent': 'WatchfulEye-NewsBot/2.0'
        }
        try:
    ...
    def build_enhanced_prompt(self, articles: List[Dict], topic: str = None) -> str:
        ...
        topic_focus = topic or (articles[0].get('diatom_topic') if articles and 'diatom_topic' in articles[0] else None)
...
DELIVER INSTITUTIONAL-GRADE INTELLIGENCE NOW."""
    ...
    def generate_investment_ideas(self, prompt: str) -> Optional[Dict]:
        ...
        # Determine which API to use (OpenRouter or OpenAI)
        if self.config.openrouter_api_key and self.config.openrouter_model:
            url = "https://openrouter.ai/api/v1/chat/completions"
            headers = {
                "Authorization": f"Bearer {self.config.openrouter_api_key}",
                "Content-Type": "application/json",
                "HTTP-Referer": os.environ.get('WATCHFULEYE_BASE_URL', 'http://localhost:5002')
            }
...
OUTPUT ONLY VALID JSON - NO OTHER TEXT."""
        ...
        try:
            ...
            logger.error(f"OpenAI HTTP error: {e}. Status: {e.response.status_code}. Response: {error_content}")
            if e.response.status_code == 429:
                logger.error("OpenAI rate limit exceeded")
            elif e.response.status_code == 401:
                logger.error("OpenAI authentication failed - check API key")
            elif e.response.status_code == 400:
                logger.error(f"OpenAI bad request: {error_content}")
            raise
    ...
    def format_message(self, structured_data: Dict, processing_time: float = 0) -> str:
        ...
        message_parts.extend([
            ...
            f"🌐 *Full analysis:* {os.environ.get('WATCHFULEYE_BASE_URL', 'http://localhost:5002')}",
    ...
    def log_output(self, formatted_message: str, raw_analysis_data: Dict, article_count: int, processing_time: float, articles_used_in_brief: List[Dict]):
        ...
        try:
            ...
            with open(formatted_file, 'w', encoding='utf-8') as f:
                f.write(f"=== DIATOMSAI INTELLIGENCE BRIEF ===\n")
    ...
    def send_discord(self, message: str) -> bool:
        ...
        try:
            ...
            data = {
                ...
                'username': 'WatchfulEye Intelligence Bot',
    ...
    def send_pushover(self, message: str) -> bool:
        ...
        try:
            data = {
                ...
                'message': message,
                'title': '🌍 DiatomsAI Intelligence Alert',
                'priority': 0,
    ...
    def run_workflow(self):
        ...
        try:
            ...
            topic = articles[0].get('diatom_topic') if articles and 'diatom_topic' in articles[0] else None
            prompt = self.build_enhanced_prompt(articles, topic=topic)
            if not prompt:
            ...
            if not investment_ideas:
                ...
                return
            # Step 4: Store analysis with topic for frontend
            try:
...
def main():
    ...
    try:
        logger.info("🌍 Starting DiatomsAI Enhanced News Intelligence System...")
